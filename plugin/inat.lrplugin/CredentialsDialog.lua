--[[
  CredentialsDialog.lua
  ---------------------
  The "Set Up Credentials" dialog, as a module.

  This was the body of PluginInit.lua, which is a menu-item script: Lightroom
  runs it top to bottom on click, so requiring it from anywhere opened the
  dialog as a side effect. Extracted here so the consolidated menu (InatMenu)
  and the Metadata panel actions can both reach it.
...
CredentialsDialog.tokenStatusText = tokenStatusText

--- Open the setup dialog. Modal, so it blocks until dismissed.
function CredentialsDialog.show()
